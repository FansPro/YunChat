...
dirs = ['emoji', 'ajmd', 'lt', 'xxy']
emoji_map = dict()

for file_path in dirs:
    emoji_map[file_path] = {}
    for file_name in os.listdir(file_path):
        if re.search(r'\.png$', file_name):
            key_name = file_name.replace('.png', '')
            emoji_map[file_path][key_name] = {
                'img': './%s/%s' % (file_path, file_name),
            }

content = json.dumps(emoji_map)
content = content.replace('"img": "', '"img": require("')
